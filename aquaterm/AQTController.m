...
#ifdef DEBUG_BOUNDS
   NSMenu *menu = [[[NSApp mainMenu] itemWithTitle:NSLocalizedString(@"Debug", @"Debug")] submenu];
   if (!menu) {
      NSMenuItem *mnuItem = [[NSApp mainMenu] addItemWithTitle:NSLocalizedString(@"Debug", @"Debug") action:NULL keyEquivalent:@""];
      NSMenu *submenu = [[NSMenu alloc] initWithTitle:NSLocalizedString(@"Debug", @"Debug")];
      mnuItem.submenu = submenu;
      menu = mnuItem.submenu;
   }
   [[menu insertItemWithTitle:NSLocalizedString(@"Show bounds", @"Show bounds") action:@selector(toggleShowBounds:) keyEquivalent:@"" atIndex:0] setTarget:nil];
#endif
}
