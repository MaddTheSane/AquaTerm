...
-(void)_aqtLabelUpdateCache
{
      ...
      normalFont = [NSFont systemFontOfSize:fontSize]; // Fall back to a system font 
   {
      ...
      if ([string isKindOfClass:[NSString class]]) {
         ...
         ctLine = [[AQTCoreTextLine alloc] initWithAttributedString:string label:self normalFont:normalFont];
      }
      if (ctLine) {
         [self _setCache:ctLine];
         return;
      }
   }
...
-(void)_aqtPathUpdateCache
{
   ...
   scratch.lineJoinStyle = NSLineJoinStyleRound; //CM FIXME - This looks like a bug. This explains why join styles don't work in the TestView... //CM
...
static NSAffineTransformStruct AQTConvertTransformStructToNS(AQTAffineTransformStruct t)
{
...
-(NSRect)updateBounds
{
   ...
   if (fitBounds) {
      ...
      // FIXME: This is lazy beyond any reasonable measure...
      tmpBounds = [transform transformBezierPath:[NSBezierPath bezierPathWithRect:NSMakeRect(0, 0, bitmapSize.width, bitmapSize.height)]].bounds;
      self.bounds = tmpBounds;
   }
   return tmpBounds;
}
